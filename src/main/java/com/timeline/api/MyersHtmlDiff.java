...
public class MyersHtmlDiff {
    public static void main(String[] args) {
        String html1 = "<p>Hello, world!</p><p>This is a test.</p><p>deleted mmm</p>";
        String html2 = "<p>Hello, timeline!</p><p>inserted</p><p>This is a test of the diff tool.</p>";
    ...
    private static List<Edit> computeMyers(List<String> token1, List<String> token2) {
        int n = token1.size();
        int m = token2.size();
        int max = n + m;
        ...
        for (int d = 0; d <= max; d++) {
            ...
            for (int k = -d; k <= d; k += 2) {
                ...
                while (x < n && y < m && token1.get(x).equals(token2.get(y))) {
                    x++;
                    y++;
                }
                ...
                if (x >= n && y >= m) {
                    return backtrack(trace, token1, token2, max, d);
                }
            }
        }
        return null;
